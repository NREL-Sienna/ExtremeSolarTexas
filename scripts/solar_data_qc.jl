...
actuals_ts = h5open("/Users/jdlara/cache/blue_texas/input_data/Solar/5-minute solar actuals/Greyhound Solar.h5", "r") do file
...
rt_output = h5open(joinpath("/Volumes/VM_WIN/Quantile data", "Greyhound Solar.h5"), "r") do file
    return read(file, "Power")[ix:ix + 36*12 - 1,  1, 50]
end

time_stamp_da = range(date, length = look_ahead_hours, step = Hour(1))
da_trace = PlotlyJS.scatter(; x = time_stamp_da, y = da_forecast, name = "DA", mode = "lines", line_shape = "hv")
PlotlyJS.plot(da_trace)

time_stamp_actuals = range(date, length = (look_ahead_hours*12), step = Minute(5))
actual_trace = PlotlyJS.scatter(; x = time_stamp_actuals, y = actual, model = "Line", name = "Actual")

rt_trace = PlotlyJS.scatter(; x = time_stamp_actuals, y = rt_output, model = "Line", name = "Real Time")
