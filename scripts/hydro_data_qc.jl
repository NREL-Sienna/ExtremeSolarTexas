...
hour_trace = PlotlyJS.scatter(; x = time_stamp_hour, y = data, name = "DA", mode = "lines", line_shape = "hv")

time_stamp_rt = range(DateTime("2018-01-01"), length = 8760*12, step = Minute(5))
data = hydro_bus_data_real_time["FALCON HEIGHTS 1 2"]
intrahour_trace = PlotlyJS.scatter(; x = time_stamp_rt, y = data, name = "actuals", mode = "lines")
...
da_trace = PlotlyJS.scatter(; x = time_stamp_ha, y = da, name = "da_trace", mode = "lines", line_shape = "hv")
...
ha_ = [ha_plant_forecast["FALCON HEIGHTS 1 2"][date][1:12] for date in range(DateTime("2018-03-30"), length = 36, step = Hour(1))]
ha = Vector{Float64}()
for i in 1:36
    push!(ha, ha_[i]...)
end
