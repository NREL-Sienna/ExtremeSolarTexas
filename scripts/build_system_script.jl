...
####################################### Load Time Series ###################################
h5open(load_time_series, "r") do file
...
for gen in get_components(HydroGen, sys)
   ...
   data = hydro_bus_data_day_ahead[bus_name]
   ...
   for ix in 1:day_count
     ...
     day_ahead_forecast[initial_time + (ix - 1)*da_interval] = forecast./gen_peak
   end
   forecast_data = Deterministic(
                    name = "max_active_power",
                    resolution = Hour(1),
...
for lz in load_zones
   ...
   wind_data_hour_ahead[lz] = vcat(temp_data, temp_data[end-12:end])
